...
//        Min 16mb: 3.695833, tjs: 4.832564, jq:4.753848, time: 24
//
//        if (++steps && c < ORD_L_1) { /* sum: 60.04 */
//          if (++steps && c === ORD_SPACE) return this.__plusOne(WHITE); /* 29.71 */
//          if (++steps && c < ORD_OPEN_PAREN) { /* sum: 9.209999999999999 */
...
//          }
//        } else { /* sum: 39.93999999999999 */
//          if (++steps && c < ORD_L_A) { /* sum: 15.139999999999999 */
//            if (++steps && c < ORD_L_A_UC) { /* sum: 9.089999999999998 */
...
//        Min 16mb: 3.708942, tjs: 5.053481, jq:4.964449, time: 25
//
//        if (++steps && c < ORD_DASH) { /* sum: 52.29 */
//          if (++steps && c === ORD_SPACE) return this.__plusOne(WHITE); /* 29.71 */
//          if (++steps && c < ORD_OPEN_PAREN) { /* sum: 9.209999999999999 */
//            if (++steps && c === ORD_CR) return this.__parseCR(); /* 5.14 */
//            if (++steps && c === ORD_TAB) return this.__plusOne(WHITE); /* 1.89 */
...
//        Min tjs: 4.037114881683366, min jq:3.9036140700930066, time: 28 unchecked
//
//        if (++steps && c < ORD_PLUS) { /* sum: 48.440000000000005 */
//          if (++steps && c < ORD_FF) { /* sum: 1.89 */
//            if (++steps && c === ORD_TAB) return this.__plusOne(WHITE); /* 1.89 */
//            if (++steps && c === ORD_LF) return this.__newline(); /* 0 */
//            if (++steps && c === ORD_VTAB) return this.__plusOne(WHITE); /* 0 */
...
//            if (++steps && c === ORD_FF) return this.__plusOne(WHITE); /* 0 */
//          }
//        } else { /* sum: 52.06999999999999 */
//          if (++steps && c < ORD_L_A) { /* sum: 27.27 */
//            if (++steps && c < ORD_L_0) { /* sum: 11.07 */
//              if (++steps && c === ORD_DOT) return this.__parseDot(); /* 6.59 */
//              if (++steps && c === ORD_COMMA) return this.__plusOne(PUNCTUATOR); /* 3.24 */
//              if (++steps && c === ORD_PLUS) return this.__parseSameOrCompound(c); /* 0.61 */
//              if (++steps && c === ORD_FWDSLASH) return this.__parseFwdSlash(expressionStart); /* 0.41 */
//              if (++steps && c === ORD_DASH) return this.__parseSameOrCompound(c); /* 0.22 */
//            } else { /* sum: 15.67 */
//              if (++steps && c < ORD_L_A_UC) { /* sum: 9.62 */
//                if (++steps && c === ORD_SEMI) return this.__plusOne(PUNCTUATOR); /* 3.71 */
//                if (++steps && c === ORD_IS) return this.__parseEqualSigns(); /* 3.13 */
//                if (++steps && c === ORD_COLON) return this.__plusOne(PUNCTUATOR); /* 1.05 */
//                if (++steps && c === ORD_L_0) return this.__parseZero(); /* 0.53 */
...
//        }
        ...
        // Min tjs: 4.0958423285602334, min jq:3.9609414389770325, time: 42 unchecked
...
//        }
    ...
    nextTokenIfElse: function(c, expressionStart, pos){
      ...
      try {
        ...
        // TOFIX: should ORD_LF go with CR? because OSX...
        if (++steps && ((c ^ ORD_PS) <= 1) /*c === ORD_PS || c === ORD_LS*/) return this.__newline();
    ...
    nextTokenIndexof: function(c, expressionStart){

      if ([ORD_SPACE, ORD_TAB, ORD_FF, ORD_VTAB, ORD_NBSP, ORD_BOM].indexOf(chr) >= 0) return this.__plusOne(WHITE);
      if ('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_$'.indexOf(chr) >= 0) return this.__parseIdentifier();
      ...
      if ([ORD_PLUS, ORD_AND, ORD_OR, ORD_DASH].indexOf(chr) >= 0) return this.__parseSameOrCompound(c);
      if ([ORD_LT, ORD_GT].indexOf(chr) >= 0) return this.__parseLtgtPunctuator(c);
      ...
      if ('123456789'.indexOf(chr) >= 0) return this.__parseNumber();
