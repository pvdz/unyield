...
//            blist[p.stmtStart.black-1].value = ')' + blist[p.stmtStart.black-1].value;
    ...
    var funcs = currentYields.map(function(yToken, index){
      ...
      yToken.returnCode = 'return ('+prefix+'ielded=true, '+prefix+'nextId = '+yToken.yieldId+', '+(yieldExpr.replace(/ /g,'') ? yieldExpr : undefined)+')\n';
...
//    });
    ...
    function processSlice(start, end, whiling) {
        ...
        funcs.forEach(function(o,index){
          ...
          if (j < o.stmtStart) {
            ...
            if (whiling && t.isBreakKeyword) {
              ...
              // only in partial loop if yield is within current range
              if (o.white > start && o.white < end) {
                fx.push('break;');
              } else {
                fx.push('continue;');
              }
      ...
      if (whiling && foundYield) {
        ...
        funcs.forEach(function(o, index){
          // make sure the yield for each func is indeed within "this" while range
          if (o.white > start && o.white < end) {
            o.output.push(
    ...
    func.lhc.value += ('\n'+
      ...
      funcs.map(function(o){
        return '\t\t'+prefix+'funcs['+o.id+'] = function '+prefix+'f'+o.id+'('+prefix+'ieldValue){\n'+
